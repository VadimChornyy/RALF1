                        ...
                        for uuu in range(Nhh):
                            if zz>=0:
                                ...
                                for kk in range(Ndel):
                                    ...
                                    for k in range(Ndel0):
                                        ...
                                        for ll in range(NCh0):
                                            ...
                                            mDD4[:,ll]= r5[ll+k+kk:ll+k+kk+NCh]*(1-(mDD4[:,ll]<D*Koe))  
                                            
                                        dQ4mn=np.mean(dQ4)#*(mDD4<D*Koe))
                                        if not (np.isnan(dQ4mn)+np.abs(dQ4mn)==np.Inf): 
                                            dQ4=dQ4-dQ4mn    
                                            dQ4_A=np.asarray(XFilter.RALF1FilterX(  dQ4*(1-(dQ4<0))+mDD4,len(dQ4),len(dQ4[0]),1,0)+dQ4mn/2,np.float16)
                                            dQ4_B=np.asarray(( -XFilter.RALF1FilterX( -dQ4*(1-(dQ4>0))+mDD4,len(dQ4),len(dQ4[0]),1,0))+dQ4mn/2,np.float16)
                                ...
                                if not (np.isnan(np.mean(dQ3mx))+np.isnan(np.mean(dQ3mn))+np.isnan(dQ4mn)+np.abs(dQ4mn)==np.Inf):            
                                    dQ3A=(dQ3mx+dQ3mn)*(1-(mDD<D*Koe))+dQ3_0*((mDD<D*Koe))                
                                    dQ4mn=np.mean(dQ3A)#*((mDD<D*Koe)))  
                                    if not (np.isnan(dQ4mn)+np.abs(dQ4mn)==np.Inf): 
                                        dQ3A=dQ3A-dQ4mn 
                                        dQ3B=dQ3A-dQ3A*np.asarray(dQ3A<0,int)       
                                        dQ2X=XFilter.RALF1FilterX(dQ3B+mDD,sz,sz,1,0)+dQ4mn/2
                                        dQ3C=-(dQ3A-dQ3A*np.asarray(dQ3A>0,int))      
                                        dQ2Y=-XFilter.RALF1FilterX(dQ3C+mDD,sz,sz,1,0)+dQ4mn/2
                                        if not (np.isnan(np.mean(dQ2X))+np.isnan(np.mean(dQ2Y))): 
                                            dQ3=np.asarray((dQ2X+dQ2Y)*(1-(mDD<D*Koe))+dQ3_0*((mDD<D*Koe)) ,np.float16)
                                            annn=0                                 
                                        else:
                                            annn=1
                                else:
                                    annn=1
                                if annn>0 or (np.isnan(dQ4mn)+np.abs(dQ4mn)==np.Inf):
                                    annn=1
                                    zz=-1
