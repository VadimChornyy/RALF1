                ...
                while kk <Ndel-1 and xxx==0:  
                    ...
                    while k<Ndel0-1 and xxx==0:     
                        ...
                        if nNxA_>0:  
                            ...
                            dQ4_A= -(seqA0_)*(mDD4_A+dQ4)-seqA0_*np.asarray( -XFilter(((mDD4_A))+(dQ4),len(dQ4),len(dQ4[0]),1,0),np.float16)
                            dQ4_B=  (seqA0_)*(mDD4_B-dQ4)+seqA0_*np.asarray( -XFilter(((mDD4_B))-(dQ4),len(dQ4),len(dQ4[0]),1,0),np.float16)
    ...
    while (not ww):
        ...
        if not PP==0:                                            
            ...
            try:
                ...
                if 100*scp.pearsonr(seq0,seq0_)[0]>0 and not abs(P[0]-1)>0.5 :
                    dd_CC=(dd_CC-P[1])/P[0] 
                else:
                    PP=0
            except:
        ...
        if len(areza)<Numproc:             
            areza.append(np.asarray([aMx, aMn],float))
            
            www=0
            while (not www):
                try:
                    hkl.dump(areza,"maxmin.rlf2")
                    www=1
                except:
                    tm.sleep(0.6)  
